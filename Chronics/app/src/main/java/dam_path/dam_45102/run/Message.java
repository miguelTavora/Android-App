...
public class Message extends AppCompatActivity {
    ...
    private EditText msgUser;
    private ArrayList<ImageButton> controlEmpty;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        msgUser = findViewById(R.id.message);
        controlEmpty = new ArrayList<>();
        hasContent = false;

        TextView otherUserTxt = findViewById(R.id.other_user);
        otherUserTxt.setText(otherUser);
        ...
        send.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if(hasContent) {
                    controlEmpty.get(0).setVisibility(View.GONE);
                    controlEmpty.remove(0);
                }
                sendMessage();
                hasContent = true;
            }
    ...
    private void setTextBetweenUsers(){
        dbRef.child(UserInfo.USERS_ACCESS).child(UserInfo.username).addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                if(dataSnapshot.hasChild(UserInfo.USER_CONVERSATIONS_ACCESS)){
                    for (DataSnapshot snapshot : dataSnapshot.child(UserInfo.USER_CONVERSATIONS_ACCESS).child(otherUser).getChildren()) {
                        hasContent = true;
    ...
    private void setPhotoToUser(){
        dbRef.child(UserInfo.USERS_ACCESS).child(otherUser).addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                if(dataSnapshot.hasChild(UserInfo.PROFILE_PICTURE_ACCESS)){
                    ImageView photo = findViewById(R.id.photo);
                    Picasso.with(Message.this).load(String.valueOf(dataSnapshot.child(UserInfo.PROFILE_PICTURE_ACCESS).getValue())).fit().centerInside().into(photo);
                }
            }
    ...
    private void sendMessage(){
        dbRef.child(UserInfo.USERS_ACCESS).child(UserInfo.username).addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                if(dataSnapshot.hasChild(UserInfo.USER_CONVERSATIONS_ACCESS)) {
                    long count = dataSnapshot.child(UserInfo.USER_CONVERSATIONS_ACCESS)
                            .child(otherUser).getChildrenCount();
                    dbRef.child(UserInfo.USERS_ACCESS).child(UserInfo.username).child(UserInfo.USER_CONVERSATIONS_ACCESS)
                            .child(otherUser).child(""+count).child(UserInfo.MESSAGE_GROUP_ACCESS).setValue(msgUser.getText().toString());
                    dbRef.child(UserInfo.USERS_ACCESS).child(UserInfo.username).child(UserInfo.USER_CONVERSATIONS_ACCESS)
                            .child(otherUser).child(""+count).child(UserInfo.USER_GROUP_ACCESS).setValue(UserInfo.username);
                    dbRef.child(UserInfo.USERS_ACCESS).child(otherUser).child(UserInfo.USER_CONVERSATIONS_ACCESS)
                            .child(UserInfo.username).child(""+count).child(UserInfo.USER_GROUP_ACCESS).setValue(UserInfo.username);
                    dbRef.child(UserInfo.USERS_ACCESS).child(otherUser).child(UserInfo.USER_CONVERSATIONS_ACCESS)
                            .child(UserInfo.username).child(""+count).child(UserInfo.MESSAGE_GROUP_ACCESS).setValue(msgUser.getText().toString());

                    createUserMessage(msgUser.getText().toString());
                    msgUser.setText("");
                    addEmptySpace();
                    scroll.post(new Runnable() {
                        @Override
                    ...
                    dbRef.child(UserInfo.USERS_ACCESS).child(UserInfo.username).child(UserInfo.USER_CONVERSATIONS_ACCESS)
                            .child(otherUser).child("0").child(UserInfo.MESSAGE_GROUP_ACCESS).setValue(msgUser.getText().toString());
                    dbRef.child(UserInfo.USERS_ACCESS).child(UserInfo.username).child(UserInfo.USER_CONVERSATIONS_ACCESS)
                            .child(otherUser).child("0").child(UserInfo.USER_GROUP_ACCESS).setValue(UserInfo.username);
                    dbRef.child(UserInfo.USERS_ACCESS).child(otherUser).child(UserInfo.USER_CONVERSATIONS_ACCESS)
                            .child(UserInfo.username).child("0").child(UserInfo.MESSAGE_GROUP_ACCESS).setValue(msgUser.getText().toString());
                    dbRef.child(UserInfo.USERS_ACCESS).child(otherUser).child(UserInfo.USER_CONVERSATIONS_ACCESS)
                            .child(UserInfo.username).child("0").child(UserInfo.USER_GROUP_ACCESS).setValue(UserInfo.username);
